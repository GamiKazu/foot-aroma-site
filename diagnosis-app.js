(() => {
  ...
  app.innerHTML = `
    <style>
      ...
      .dq-hero-visual{position:relative;isolation:isolate}.dq-hero-visual img{display:block;width:100%;height:auto;aspect-ratio:16/9;object-fit:cover}.dq-hero-visual::after{content:"";position:absolute;inset:auto 0 0;height:38%;z-index:1;background:linear-gradient(180deg,transparent,rgba(29,48,34,.42));pointer-events:none}
      .dq-hero-action{position:absolute;z-index:2;left:50%;bottom:34px;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;width:min(88%,440px)}.dq-hero-action .dq-button{width:100%;margin:0;padding:18px 32px;background:rgba(48,75,53,.96);border:1px solid rgba(255,255,255,.42);box-shadow:0 12px 34px rgba(24,38,27,.25);backdrop-filter:blur(8px)}.dq-hero-action .dq-button:hover{background:#29472f;transform:translateY(-2px)}.dq-hero-action .dq-note{margin:10px 0 0;color:#fff;text-shadow:0 1px 6px rgba(0,0,0,.45)}
      ...
      @media(max-width:768px){.dq-hero{padding:18px 12px 34px}.dq-form-section,.dq-result-section{padding:36px 18px}.dq-hero-card{border-radius:22px}.dq-form,.dq-result-card{padding:28px 20px;border-radius:24px}.dq-hero-visual img{aspect-ratio:auto;object-fit:contain}.dq-hero-visual::after{height:42%}.dq-hero-action{bottom:14px;width:calc(100% - 28px)}.dq-hero-action .dq-button{padding:14px 18px;font-size:.92rem}.dq-hero-action .dq-note{font-size:.68rem;margin-top:6px}.dq-options,.dq-options.six{grid-template-columns:1fr}.dq-result-grid,.dq-summary{grid-template-columns:1fr}.dq-question{padding:22px 0}.dq-option label{min-height:54px}.dq-button{width:100%}.dq-nav .dq-button{min-width:0;margin-top:18px}.dq-retry{margin-left:0}.dq-score strong{font-size:4rem}}
    </style>
    <section class="dq-hero"><div class="container"><div class="dq-hero-card">
      <div class="dq-hero-visual">
        ...
        <div class="dq-hero-action"><a class="dq-button" href="#question-check">無料で診断をはじめる</a><p class="dq-note">登録不要・写真のアップロード不要・約3分</p></div>
      </div>
    </div></div></section>
